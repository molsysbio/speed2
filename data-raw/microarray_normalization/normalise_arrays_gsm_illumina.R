...
platformtmp<-read.table("platform.dat");
platform=platformtmp[1,1];
gpl=Table(getGEO(platform))
geneIDcolumn <- (labels(gpl)[[2]]=="Gene.ID") | (labels(gpl)[[2]]=="ENTREZ_GENE_ID") |  (labels(gpl)[[2]]=="Entrez_Gene_ID");
# assert:
stopifnot(sum(geneIDcolumn)>0);

geneIDs <- gpl[geneIDcolumn];
probesets=as.character(gpl$Search_Key);
